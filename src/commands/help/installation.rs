...
pub fn register() -> CreateCommand {
    CreateCommand::new("installation")
        .description("Tells someone to read the installation guide")
        .add_option(
            CreateCommandOption::new(
                CommandOptionType::User,
                "member",
                "The member to tell to read the installation guide",
            )
            .required(true),
        )
}
...
pub async fn handle(options: &[ResolvedOption<'_>]) -> CreateInteractionResponse {
    ...
    let (content, embed, components, is_ephemeral) =
        if let ResolvedValue::User(user, _) = target_user {
            ...
            let button =
                CreateButton::new_link("https://meteorclient.com/faq/installation").label("Guide");
